...
public class ConveyorDeliverySystem : MonoSingleton<ConveyorDeliverySystem>, IConveyorPickupHandler
{
    ...
    [SerializeField] private List<Cube> cachedMovers = new();
    ...
    [SerializeField] private Transform pickupColliderRoot;
    [SerializeField] private float pickupColliderWidth = 2f;
    [SerializeField] private float pickupColliderHeight = 0.4f;
    [SerializeField] private float pickupColliderDepth = 0.3f;
    ...
    private readonly List<AnimCube> _activeAnimCubes = new();
    private readonly List<DeliveryCubeState> _deliveryStates = new();
    private readonly List<CarrierBase> _activeCarriers = new();
    private readonly List<float> _cachedCarrierPickupProgresses = new();
    ...
    public void RequestPlayLoopSound()
    {
        ...
        if (activeCount == 1)
        {
            SoundManager.Instance?.PlayLoop(AudioClipName.sfx_merge_loop);
        }
    }
    ...
    protected override void Awake()
    {
        ...
        _deliveryCubeFactory ??= new ConveyorDeliveryCubeFactory();
        ...
        int undoBatchId)
    {
        ...
        BoosterUndoSystem.Instance.NotifyCubeSpawnedOnConveyor(undoBatchId, cube);
        ...
        out CarrierReceiveReservation reservation)
    {
        ...
        BoosterUndoSystem.Instance.NotifyCubePickupStarted(
            state.UndoBatchId,
            state.Cube,
            targetCarrier,
    ...
    private bool IsCarrierLockedForWinCheck(CarrierBase carrier, HashSet<int> unlockableContainerIds)
    {
        var member = carrier.GetComponent<CarrierContainerMember>();
        if (member == null || member.CurrentContainer == null) return false;
        if (unlockableContainerIds.Contains(member.CurrentContainer.ContainerId)) return false;
        return !member.CurrentContainer.IsOpen;
    }
    ...
    {
        // 0. Xác định các container được phép mở khóa (đã mở hoặc sắp mở)
        var unlockableContainerIds = new HashSet<int>();
        var activeContainers = ContainerMechanic.GetActiveContainers();
        if (activeContainers != null)
        {
            foreach (var container in activeContainers)
            {
                if (container != null && (container.IsOpen || container.IsAssignedToUnlock))
                {
                    unlockableContainerIds.Add(container.ContainerId);
                }
            }
        }

        // Quét thêm các block đang unloading xem có chứa key mở container nào không
        if (_activeCarriers != null)
        {
            for (var i = 0; i < _activeCarriers.Count; i++)
            {
                ...
                for (var j = 0; j < blocks.Count; j++)
                {
                    ...
                    {
                        int targetContainerId = block.GetKeyTargetContainerId();
                        if (targetContainerId != -1)
                        {
                            unlockableContainerIds.Add(targetContainerId);
                        }
                    }
                }
            }
        ...
        // 5.1 Thống kê số lượng cube còn lại trong queue của Spawner
        if (_activeCarriers != null)
        {
            for (var i = 0; i < _activeCarriers.Count; i++)
            {
                var carrier = _activeCarriers[i];
                if (carrier is Spawner spawner)
                {
                    // Nếu spawner bị khóa và không thể mở trong lượt này, bỏ qua
                    if (IsCarrierLockedForWinCheck(spawner, unlockableContainerIds)) continue;

                    var queue = spawner.BlocksQueue;
                    if (queue != null)
                    {
                        // Xác định xem block hiện tại của Spawner có đang Unload hay không
                        bool isCurrentUnloading = spawner.SingleBlock != null && spawner.SingleBlock.IsOpened;
                        int startIndex = isCurrentUnloading ? spawner.CurrentQueueIndex + 1 : spawner.CurrentQueueIndex;

                        for (int k = startIndex; k < queue.Count; k++)
                        {
                            var blockData = queue[k];
                            if (blockData != null && blockData.HasContent)
                            {
                                var blockColor = blockData.BlockColorType;
                                if (blockColor != EBlockColorType.None && blockData.CubeCount > 0)
                                {
                                    conveyorCubes.TryGetValue(blockColor, out int val);
                                    conveyorCubes[blockColor] = val + blockData.CubeCount;
                                }
                            }
                        }
                    }
        ...
        if (_activeCarriers != null)
        {
            for (var i = 0; i < _activeCarriers.Count; i++)
            {
                ...
                if (carrier is Spawner) continue;
                ...
                if (IsCarrierLockedForWinCheck(carrier, unlockableContainerIds)) continue;
