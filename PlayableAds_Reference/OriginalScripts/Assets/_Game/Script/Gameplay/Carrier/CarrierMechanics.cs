...
public readonly struct CarrierActionBlock
{
...
public readonly struct CarrierActionEvaluationResult
{
...
public readonly struct CarrierFinishedColorEvent : ICarrierMechanicEvent
{
...
{
    private readonly List<ICarrierMechanicRuntime> _mechanics = new();
    ...
    private static ICarrierMechanicRuntime CreateMechanicRuntime(CarrierMechanicData mechanicData)
    {
        ...
        return mechanicData.Type switch
        {
            ECarrierMechanic.HiddenByColor => new HiddenCarrierByColorMechanicRuntime(mechanicData.UnlockColor),
            ECarrierMechanic.OneWay => new OneWayCarrierMechanicRuntime(),
            ECarrierMechanic.SpecialColorReceiver => new SpecialColorReceiverMechanicRuntime(
                mechanicData.TargetColor != EBlockColorType.None ? mechanicData.TargetColor : mechanicData.UnlockColor),
            _ => null,
        };
    }
}
...
public sealed class CarrierActionGateResolver
{
    ...
    private CarrierActionEvaluationResult Evaluate(Carrier carrier, ECarrierActionType actionType, EBlockColorType? colorType)
    {
        ...
        {
            CarrierActionBlock? nextBlock = actionType switch
            {
                ECarrierActionType.Interact => mechanic is ICarrierInteractRuleProvider interactProvider
                    ? interactProvider.GetInteractBlock(carrier)
                    : null,
                ECarrierActionType.Unload => mechanic is ICarrierUnloadRuleProvider unloadProvider
                    ? unloadProvider.GetUnloadBlock(carrier)
                    : null,
                ECarrierActionType.Receive => mechanic is ICarrierReceiveRuleProvider receiveProvider && colorType.HasValue
                    ? receiveProvider.GetReceiveBlock(carrier, colorType.Value)
                    : null,
                _ => null
            };
...
public sealed class CarrierVisualResolver
{
    public CarrierVisualRequest Resolve(Carrier carrier)
    {
        ...
        foreach (var mechanic in carrier.MechanicContainer.Mechanics)
        {
            if (mechanic is not ICarrierVisualRequestProvider requestProvider) continue;
