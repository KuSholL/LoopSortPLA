...
public sealed class CarrierVisualController
{
    ...
    private void ClearVisual(bool animate = true)
    {
        ...
        if (Application.isPlaying)
        {
            if (animate)
            {
                ...
                PlayAndPoolAsync(visualToClear).Forget();
            }
            else
            {
    ...
    private async UniTaskVoid PlayAndPoolAsync(CarrierMechanicVisual visual)
    {
        if (visual == null) return;
        await visual.PlayDisappearAnimationAsync();
    ...
    private void SetCarrierRenderersVisible(bool isVisible)
    {
        ...
        {
            var carrier = _carrierRoot != null ? _carrierRoot.GetComponent<CarrierBase>() : null;
            var member = carrier != null ? carrier.GetComponent<CarrierContainerMember>() : null;
            bool isLockedByContainer = member != null && member.IsLocked && (member.CurrentContainer == null || !member.CurrentContainer.IsLidOpening);
