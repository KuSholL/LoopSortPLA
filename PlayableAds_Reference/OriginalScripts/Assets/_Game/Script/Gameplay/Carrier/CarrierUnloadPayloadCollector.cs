...
public sealed class CarrierUnloadPayloadCollector
{
    ...
    public bool TryCollect(int maxUnloadCubeCount, out CarrierUnloadRequest unloadRequest)
    {
        ...
        unloadRequest = new CarrierUnloadRequest(_carrier, payloads, sourceBlocks);
        return true;
    }
    ...
    private List<CarrierCubePayload> CollectPayloadsFromMatchingBlocks(
        ...
        List<UndoSourceBlockSnapshot> sourceBlocks,
        List<Block> unloadingBlocks)
    {
        ...
        while (_carrier.BlockController.TryGetCurrentMatchingBlock(unloadColorType, out var block))
        {
            ...
            AddSourceBlockSnapshot(block, sourceBlocks);
    ...
    private void AddSourceBlockSnapshot(Block block, List<UndoSourceBlockSnapshot> sourceBlocks)
    {
        if (block == null || sourceBlocks == null) return;

        var blockIndex = _carrier.BlockController.GetBlockIndex(block);
