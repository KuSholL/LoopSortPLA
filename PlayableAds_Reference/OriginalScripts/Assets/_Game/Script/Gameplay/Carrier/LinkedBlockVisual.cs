...
public sealed class LinkedBlockVisual : MonoBehaviour, IClickableObject
{
    [SerializeField] private MeshRenderer meshRenderer;
    [SerializeField] [ShowIf(nameof(IsBlock4X))] private SkinnedMeshRenderer skinnedMeshRenderer;
    [SerializeField] private GameObject modelGO;
    [SerializeField] private EBlockShapeType shapeType;
    [SerializeField] [ShowIf(nameof(IsBlock4X))] private SpriteRenderer catFace;
    ...
    public bool CanSelectAsSource()
    {
        return _carrier != null
               && _anchorBlock != null
               && CanPassTutorialBlockRule()
               && ClawTransferUtility.CanSelectSource(_anchorBlock, _carrier);
    }
    ...
    {
        if (!_carrier || !_anchorBlock || BoosterSystem.Instance == null) return;
        BoosterSystem.Instance.SelectBlock(transform.position, _anchorBlock, _carrier);
    }

    public bool CanBeClicked()
    {
        return BoosterSystem.Instance
    ...
    public void OnClickBlocked()
    {
        if (BoosterSystem.Instance == null || BoosterSystem.Instance.ClawMode != EClawSelectionMode.SelectTargetCarrier)
            return;
        if (_carrier == null || !_carrier.CanBeClawTarget()) return;
        if (GameEventBus.CanSelectTutorialClawCarrier != null
            && !GameEventBus.CanSelectTutorialClawCarrier.Invoke(_carrier)) return;
        BoosterSystem.Instance.SelectCarrier(transform.position, _carrier);
    }

    private bool CanPassTutorialBlockRule()
    {
        if (GameEventBus.CanSelectTutorialClawBlock == null) return true;
        return GameEventBus.CanSelectTutorialClawBlock.Invoke(_anchorBlock, _carrier);
    }
}
