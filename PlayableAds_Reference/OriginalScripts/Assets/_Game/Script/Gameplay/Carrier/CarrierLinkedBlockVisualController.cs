...
public sealed class CarrierLinkedBlockVisualController
{
    ...
    private readonly Dictionary<int, List<LinkedBlockVisual>> _visualPool = new();
    ...
    public bool TryPlayBlockedFullAnimation(Block anchorBlock)
    {
        ...
        foreach (var group in _activeGroups)
        {
            ...
            if (group.BlockCount is < 2 or > 3) return false;
        ...
        bool forceFullAnimation)
    {
        ...
        var hasKey = false;
        var keyColorType = EBlockColorType.None;
        Block keyBlock = null;
        foreach (var b in blocks)
        {
            if (b.HasVisibleContainerKey())
            {
                hasKey = true;
                keyColorType = b.GetResolvedKeyColor(b.GetBlockColorType());
                keyBlock = b;
                break;
            }
        }

        visual.Apply(colorEntry, catColorEntry, suppressPlacementAnimation, forceFullAnimation, hasKey, keyColorType);
        ...
        if (blockCount == 4 && hasKey && keyBlock != null)
        {
            keyBlock.RevealContainerKeyIfNeeded(suppressPlacementAnimation);
        }
    ...
    public void RevealKeyInLinkedGroupOfBlock(Block block)
    {
        if (block == null) return;
        for (var i = 0; i < _activeGroups.Count; i++)
        {
            ...
            if (group.Contains(block))
            {
                ...
                {
                    if (b != null && b.HasVisibleContainerKey())
                    {
                        b.RevealContainerKeyIfNeeded();
                        return;
                    }
                }
            }
