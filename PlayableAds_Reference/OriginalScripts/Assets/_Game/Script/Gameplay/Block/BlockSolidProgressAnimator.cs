...
public class BlockSolidProgressAnimator : MonoBehaviour
{
    ...
    public void PlayTriggerActiveAnimation()
    {
        ...
        if (_isProgressTweenPlaying || _progressHandle.IsActive())
        {
            StopProgressTween(true);
        }
    ...
    private void PlayProgressTween(DataAnim animData)
    {
        ...
        {
            var motion = LMotion
                .Create(animData.LocalScales[index], animData.LocalScales[index + 1], animData.Duration)
                .WithEase(animData.Ease);
            ...
            sequence.Append(motion.BindToLocalScale(animatedTarget));
        }

        _progressHandle = sequence.Run();
    }
