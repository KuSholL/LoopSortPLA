...
public class CubeMovement : MonoBehaviour, ICustomTimeScaleTarget
{
    ...
    private static readonly Dictionary<Spline, SplineDataCache> _splineCache = new();
    ...
    {
        if (_splineContainer == null || _splineContainer.Spline is not { Count: > 0 }) return _lastProgress;
    ...
    private bool CanMove()
    {
        ...
        return rb != null
               ...
               && _splineContainer.Spline is { Count: > 0 };
    }
    ...
    public void LockYAxisTemporarily(float duration, float speedMultiplier = 1f, float speedMultiplierDuration = -1f)
    {
        ...
        if (duration > 0f)
            ApplyYAxisLockAsync(++_yAxisLockVersion, duration).Forget();
        ...
        {
            ApplySpeedMultiplierAsync(
                ++_speedMultiplierLockVersion,
                actualSpeedMultiplierDuration,
                Mathf.Clamp01(speedMultiplier)).Forget();
        }
    }

    private async UniTaskVoid ApplyYAxisLockAsync(int lockVersion, float duration)
    {
        if (rb == null) return;
        ...
        if (lockVersion != _yAxisLockVersion || rb == null) return;
        SetSpawnYAxisLocked(false);
    }

    private async UniTaskVoid ApplySpeedMultiplierAsync(int lockVersion, float duration, float speedMultiplier)
    {
        ...
        if (lockVersion != _speedMultiplierLockVersion) return;
        _lockedSpeedMultiplier = 1f;
    }
