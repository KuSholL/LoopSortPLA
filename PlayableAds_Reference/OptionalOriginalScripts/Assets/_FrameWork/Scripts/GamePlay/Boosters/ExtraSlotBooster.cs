...
public class ExtraSlotBooster : BaseBooster
{
    ...
    protected override BoosterState CalculateState()
    {
        ...
        if (BoosterSystem.Instance != null && !BoosterSystem.Instance.CanUseBoosterExtraSlot())
            return BoosterState.Disabled;
    ...
    {
        var currentAmount = DataManager.PlayerData.BoosterData.ExtraSlotBooster;
        
        if (UIExtraSlotBoosterLayer.IsTutorial && !RewardItemFlyEffectController.IsPlayingAnimation)
        {
    ...
    {
        if (type == RewardType.ExtraSlotBooster) RefreshVisual();
    }
    ...
    {
        var usedBooster = BoosterSystem.Instance != null && BoosterSystem.Instance.TryUseExtraSlotBooster();
        if (!usedBooster) return;
        base.ExecuteBooster();
        
        if (LevelManager.Instance.IsTutorial)
        {
            GameEventBus.FinishExtraSlotBoosterStep2?.Invoke();
            RefreshVisual();
            return;
        }
        
        DataManager.ChangeBooster(RewardType.ExtraSlotBooster, -1);
        RefreshVisual();
    }
    ...
    public override void BuyBooster()
    {
        ...
        {
            ItemType = RewardType.ExtraSlotBooster,
            Title = TextUtility.GetI2("ui_extra_slot"),
            Content = TextUtility.GetI2("popup_get_extraslot"),
    ...
    protected override void OnBuyBoosterByGold()
    {
        ...
        {
            DataManager.ChangeBooster(RewardType.ExtraSlotBooster, _config.AmountBuy, source: "ingame_gold");
    ...
    protected override void OnBuyBoosterByAds()
    {
        ...
        return;
#endif
        AdmobManager.Instance.TryRequestShowRewardedVideo("ad_booster_extraslot", OnVideoRewarded, null);
    }
    ...
    private void OnVideoRewarded()
    {
        GameEventBus.OnReceiveBooster?.Invoke(() =>
        {
            DataManager.ChangeBooster(RewardType.ExtraSlotBooster, _config.WatchAds, source: "ingame_ads");
            LayerManager.Instance.CloseLastLayerGroup();
        });
