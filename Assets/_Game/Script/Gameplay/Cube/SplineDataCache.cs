...
public class SplineDataCache
{
    ...
    public SplineDataCache(ConveyorPathRuntime spline, int sampleCount = 200)
    {
        ...
        _isClosed = spline != null && spline.Closed;
        ...
        for (int i = 0; i < sampleCount; i++)
        {
            ...
            {
                Progress = progress,
                LocalPosition = spline != null ? spline.EvaluateLocalPosition(progress) : Vector3.zero,
                LocalTangent = spline != null ? spline.EvaluateLocalTangent(progress) : Vector3.forward
            };
        }
    }

    public void GetNearestPointLocal(Vector3 localPosition, float startProgress, int searchRange,
        out Vector3 nearestLocalPoint, out float progress, out Vector3 localTangent)
    {
        int approxIndex = Mathf.RoundToInt(startProgress * (_sampleCount - 1));
        ...
        if (distToGuessSqr > 4.0f) // 2m threshold
        {
        ...
        for (int i = -actualSearchRange; i <= actualSearchRange; i++)
        {
            ...
            if (_isClosed)
            {
                index = (index + _sampleCount) % _sampleCount;
            }
            else
            {
                index = Mathf.Clamp(index, 0, _sampleCount - 1);
            }
    ...
    public void GetNearestPointGlobal(Vector3 localPosition,
        out Vector3 nearestLocalPoint, out float progress, out Vector3 localTangent)
    {
        float minDistanceSqr = float.MaxValue;
        int bestIndex = 0;
    ...
    private void InterpolateSample(Vector3 localPosition, int bestIndex, 
        out Vector3 nearestLocalPoint, out float progress, out Vector3 localTangent)
    {
        var bestSample = _samples[bestIndex];
        ...
        var prevSample = _samples[prevIndex];
        var nextSample = _samples[nextIndex];

        var neighbor = (localPosition - prevSample.LocalPosition).sqrMagnitude < (localPosition - nextSample.LocalPosition).sqrMagnitude 
            ? prevSample 
            : nextSample;
        ...
        if (segmentLengthSqr > 0.000001f)
        {
            ...
            if (progressDiff > 0.5f) progressDiff -= 1f;
            else if (progressDiff < -0.5f) progressDiff += 1f;
