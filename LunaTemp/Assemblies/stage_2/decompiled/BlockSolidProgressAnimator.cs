...
public class BlockSolidProgressAnimator : MonoBehaviour
{
	...
	{
		progress = Mathf.Clamp01(progress);
		AnimType animType = GetAnimTypeByProgress(progress, forceFullAnimation);
		ConfigManager configManager = MonoSingleton<ConfigManager>.Instance;
		if (!(configManager == null))
		{
			AnimBlockConfig animConfig = configManager.GetAnimBlockConfig();
			if (!(animConfig == null))
			{
	...
	{
		if (isBlock4X && !(_animator == null) && !(_animator.runtimeAnimatorController == null))
		{
	...
	{
		if (animatedTarget == null || animData == null || animData.LocalScales == null || animData.LocalScales.Count < 2 || (_currentAnimType == animData.Type && _isProgressTweenPlaying))
		{
			return;
		}
		StopProgressTween(true);
		...
		for (int index = 0; index < animData.LocalScales.Count - 1; index++)
		{
			Vector3 targetScale = animData.LocalScales[index + 1];
			_progressSequence.Append(animatedTarget.DOScale(targetScale, animData.Duration).SetEase(animData.Ease));
		}
		_progressSequence.OnComplete(delegate
		{
			...
			if (isBlock4X && animData.Type == AnimType.Full)
			{
				SetAnimatorEnabled(true);
			}
			if (animData.Type == AnimType.Full)
			{
				MonoSingleton<SoundManager>.Instance?.PlayOneShot(AudioClipName.sfx_squash_end);
			}
		});
	}
