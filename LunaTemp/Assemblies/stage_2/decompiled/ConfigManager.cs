...
{
	[FormerlySerializedAs("blockConfig")]
	[SerializeField]
	private ColorConfigSO colorConfig;

	[SerializeField]
	private ColorConfigSO cubeColorConfig;

	[SerializeField]
	private CubeConfigSO cubeConfig;

	[SerializeField]
	private ColorConfigSO specialColor;

	[SerializeField]
	private CarrierConfigSO carrierConfig;

	[SerializeField]
	private CatColorConfigSO catColorConfig;

	[SerializeField]
	private AnimBlockConfig animBlockConfig;

	[SerializeField]
	private CubeMovementConfigSO cubeMovementConfig;

	[SerializeField]
	private StylizedColorConfigSO stylizedColorConfig;

	[SerializeField]
	private RemainingColorConfigSO remainingColorConfig;

	static ConfigManager()
	{
		MonoSingleton<ConfigManager>.AutoCreate = false;
	}
	...
	{
		if (!catColorConfig || catColorConfig.CatColorEntries == null)
		{
			return null;
		}
		return catColorConfig.CatColorEntries.Find((CatColorEntry x) => x.BlockColorType == colorType);
	}
}
