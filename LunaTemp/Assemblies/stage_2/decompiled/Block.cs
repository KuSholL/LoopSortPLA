...
public class Block : MonoBehaviour
{
	...
	[SerializeField]
	private EBlockColorType blockColorType;

	[SerializeField]
	private Color blockColor = Color.white;

	[SerializeField]
	private Color blockShadowColor = Color.white;
	...
	[SerializeField]
	private int visualCubes;

	[SerializeField]
	private EBlockState state = EBlockState.Idle;
	...
	private BlockOpenHandler OpenHandler
	{
		get
		{
			...
			{
				_openHandler = new BlockOpenHandler();
			}
			return _openHandler;
		}
	}
	...
	private BlockReceiveHandler ReceiveHandler
	{
		get
		{
			...
			{
				_receiveHandler = new BlockReceiveHandler();
			}
	...
	private ContainerKey ContainerKeyComponent
	{
		get
		{
			...
			{
				containerKey = GetComponent<ContainerKey>();
			}
			if (containerKey == null && Application.isPlaying)
			{
				containerKey = base.gameObject.AddComponent<ContainerKey>();
			}
			return containerKey;
		}
	}
	...
	{
		if (!_preserveRuntimeStateOnDisable)
		{
	...
	private void ConfigureMechanics(BlockData blockData)
	{
		...
		ConfigureContainerKey(GetResolvedKeyColor(blockData?.BlockColor ?? EBlockColorType.None), false);
		BlockMechanicData swapMechanic = GetBlockMechanic(EBlockMechanic.SwappingBlock);
	...
	public void SetPhysicsCollidersEnabled(bool isEnabled)
	{
		...
		for (int i = 0; i < _cachedPhysicsColliders.Length; i++)
		{
			Collider target = _cachedPhysicsColliders[i];
			if (target != null)
			{
	...
	{
		ColorEntry colorEntry = GetColorEntry(colorConfig, blockData);
		InternalClearCubes();
		...
		state = EBlockState.Idle;
		maxCubes = MonoSingleton<CapacityManager>.Instance.CubePerBlock;
	...
	public EBlockColorType GetBlockColorType()
	{
	...
	{
		return state == EBlockState.Receiving;
	}

	public bool IsLinkedVisualSuppressed()
	{
		return _isLinkedVisualSuppressed;
	}

	public bool IsHiddenForClawBooster()
	{
		return _isHiddenForClawBooster;
	}
	...
	{
		return !hasContent && currentCubes == 0 && _reservedReceiveCount == 0 && state == EBlockState.Idle;
	}

	public bool IsHiddenVisualActive()
	{
		return IsHiddenMechanicVisualActive();
	}

	public int GetKeyTargetContainerId()
	{
		return GetBlockMechanic(EBlockMechanic.KeyUnlockContainer)?.ContainerId ?? (-1);
	}

	public bool HasVisibleContainerKey()
	{
		return HasContainerKeyMechanic() && (ContainerKeyComponent == null || !ContainerKeyComponent.IsConsumed);
	}

	public bool IsReadyForFinish()
	{
		return hasContent && IsFull() && !IsHiddenMechanicVisualActive();
	}
	...
	{
		return hasContent && CanOpenByMechanic() && state != EBlockState.Unloading && _reservedReceiveCount == 0;
	}
	...
	{
		if (!CanOpenByMechanic())
		{
			return false;
		}
		bool didOpen = OpenHandler.TryOpen(currentCubes, ref state, _reservedReceiveCount);
		if (didOpen)
		{
			RefreshVisualState();
		}
	...
	public bool IsAvailableForReceive(EBlockColorType colorType)
	{
		...
		return ReceiveHandler.IsAvailableForReceive(hasContent, currentCubes, _reservedReceiveCount, state, blockColorType, colorType, maxCubes);
	}
	...
	public bool TryReserveReceive(EBlockColorType colorType, out Vector3 worldPosition)
	{
		...
		int cubeIndex = maxCubes - 1 - currentCubes;
		bool wasEmpty = !hasContent || state == EBlockState.Idle;
		bool didReserve = ReceiveHandler.TryReserveReceive(ref hasContent, ref blockColorType, ref blockColor, ref blockShadowColor, ref state, ref currentCubes, ref _reservedReceiveCount, colorType, maxCubes);
		if (didReserve)
		{
			worldPosition = BlockZigzagOffsetCalculator.GetZigzagWorldPosition(cubeIndex, maxCubes, basePos, base.transform);
			RefreshVisualState();
		}
		return didReserve;
	}
	...
	public bool TryReceiveCube(EBlockColorType colorType, Color color)
	{
		...
		bool didReceive = ReceiveHandler.TryReceiveCube(ref currentCubes, ref visualCubes, ref _reservedReceiveCount, ref hasContent, ref blockColorType, ref blockColor, ref blockShadowColor, ref state, colorType, color, maxCubes);
		if (didReceive)
		{
			RefreshVisualState();
		}
	...
	public void RestoreFullBlock(EBlockColorType colorType, Color color, Color shadowColor, int cubeCount, bool suppressProgressAnimation = false)
	{
		maxCubes = ((MonoSingleton<CapacityManager>.Instance != null) ? MonoSingleton<CapacityManager>.Instance.CubePerBlock : maxCubes);
	...
	{
		if (_isLinkedVisualSuppressed != isSuppressed)
		{
	...
	{
		if (_isHiddenForClawBooster != isHidden)
		{
	...
	private void RefreshVisualState(float progress, bool suppressProgressAnimation, bool forceFullAnimation)
	{
		if (!(BlockVisualComponent == null))
		{
			...
			{
				BlockVisualComponent.ApplyVisualState(hasContent, !_isLinkedVisualSuppressed, blockColorType, IsHiddenMechanicVisualActive(), HasVisibleContainerKey(), progress, suppressProgressAnimation, forceFullAnimation, GetResolvedKeyColor(blockColorType));
			}
		}
	}
	...
	{
		return (maxCubes > 0) ? ((float)visualCubes / (float)maxCubes) : 0f;
	}
	...
	{
		if (blockEvent is PreviousBlockReleasedEvent)
		{
	...
	{
		if (_hasHiddenMechanic && !_isHiddenRevealed)
		{
			_isHiddenRevealed = true;
		}
	}
	...
	{
		if (HasContainerKeyMechanic())
		{
			Vector3 startPosition = base.transform.position;
			Quaternion startRotation = base.transform.rotation;
			if (BlockVisualComponent != null && BlockVisualComponent.FixedVisual != null && BlockVisualComponent.FixedVisual.KeyRenderer != null)
			{
	...
	{
		ContainerKey component = ContainerKeyComponent;
		if (!(component == null))
		{
			BlockMechanicData mechanic = GetBlockMechanic(EBlockMechanic.KeyUnlockContainer);
			component.Configure(mechanic != null, colorType, mechanic?.ContainerId ?? (-1), isConsumed);
		}
	}
	...
	{
		BlockMechanicData mechanic = GetBlockMechanic(EBlockMechanic.KeyUnlockContainer);
		return (mechanic != null && mechanic.KeyColor != EBlockColorType.None) ? mechanic.KeyColor : fallbackColor;
	}
	...
	public bool HasLinkGroupId()
	{
		return GetLinkGroupId() >= 0;
	}

	public int GetLinkGroupId()
	{
		return GetBlockMechanic(EBlockMechanic.BlockLink)?.LinkGroupId ?? (-1);
	}

	public void RemoveLinkMechanic()
	{
		_runtimeMechanics.RemoveAll((BlockMechanicData m) => m.Type == EBlockMechanic.BlockLink);
	}

	private BlockMechanicData GetBlockMechanic(EBlockMechanic type)
	{
		if (_runtimeMechanics == null)
		{
			return null;
		}
		foreach (BlockMechanicData mechanic in _runtimeMechanics)
		{
			if (mechanic == null || mechanic.Type != type)
			{
				continue;
			}
			return mechanic;
	...
	private static bool HasMechanic(List<BlockMechanicData> mechanics, EBlockMechanic type)
	{
		...
		foreach (BlockMechanicData mechanic in mechanics)
		{
			if (mechanic == null || mechanic.Type != type)
			{
				continue;
	...
	{
		List<BlockMechanicData> result = new List<BlockMechanicData>();
		if (mechanics == null)
		{
			return result;
		}
	...
	public void ConfigureSwapMechanic(int swapGroupId, bool isActive)
	{
		...
		BlockVisual visual = BlockVisualComponent;
	...
	{
		if (hasContent && _isSwappingActive)
		{
			blockColorType = newColorType;
			ColorEntry entry = colorConfig.GetColorEntry(newColorType);
	...
	public void SetSwapArrowColor(EBlockColorType colorType, ColorConfigSO colorConfig = null)
	{
		...
		if (colorConfig == null && MonoSingleton<ConfigManager>.Instance != null)
		{
			colorConfig = MonoSingleton<ConfigManager>.Instance.GetColorConfig();
		}
		if (BlockVisualComponent != null)
		{
