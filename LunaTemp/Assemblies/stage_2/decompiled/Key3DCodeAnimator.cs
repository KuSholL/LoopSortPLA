...
public sealed class Key3DCodeAnimator : MonoBehaviour
{
	...
	[SerializeField]
	private Transform scissorsL;

	[SerializeField]
	private Transform scissorsR;
	...
	public void PlayFlyAndUnlockAnimation(Vector3 startPosition, Quaternion startRotation, Vector3 startScale, float flyDuration, Ease flyEase, Action onFirstCut, Action onSecondCut)
	{
		Cancel();
		StoreInitialState();
		if (rootTransform == null)
		...
		SetActiveState(true);
		Vector3 landingPosition = base.transform.TransformPoint(new Vector3(4.44f, 3.96f, 2.28f) * scaleMultiplier);
		Quaternion landingRotation = base.transform.rotation * Quaternion.Euler(0f, 51.225f, 0f);
		Vector3 landingScale = Vector3.one * (1.7707f * scaleMultiplier);
		...
		_animationSequence.AppendCallback(delegate
		{
			...
			{
				MonoSingleton<SoundManager>.Instance.PlayOneShot(AudioClipName.sfx_cut);
			}
		});
		...
		_animationSequence.AppendInterval(0.12f);
		_animationSequence.OnComplete(delegate
		{
	...
	{
		if (!_initialized && !(rootTransform == null))
		{
			_initialized = true;
	...
	{
		if (!(rootTransform == null) && _initialized)
		{
			...
			if (scissorsL != null)
			{
				scissorsL.localRotation = Quaternion.identity;
			}
			if (scissorsR != null)
			{
				scissorsR.localRotation = Quaternion.identity;
			}
